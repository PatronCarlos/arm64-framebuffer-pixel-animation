...
.equ Get_Pixel_pitch, 0x00040008 // Frame Buffer: Get Pitch
...
_start:
    mrs     x1, mpidr_el1     // X1 = Multiprocessor Affinity Register (MPIDR)
    and     x1, x1, #3        // X1 = CPU ID (Bits 0..1)
    cbz     x1, StackInit     // IF (CPU ID == 0) Branch To StackInit else (Core ID 1..3) CoreLoop
...
FB_OFFSET_Y:
    ...
    .word Set_Pixel_Order // Tag Identifier
    .word 0x00000004 // Value Buffer Size In Bytes
    .word 0x00000004 // 1 bit (MSB) Request/Response Indicator (0=Request, 1=Response), 31 bits (LSB) Value Length In Bytes
